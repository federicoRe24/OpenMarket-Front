...
function actualizarProductos(lista){
    let filtro = inputSearch.value;

    // Busco valores pasados por la URL
    if (filtro == "" && window.location.search != "") {
        filtro = (window.location.search.split("="))[1];
    }

    let listaFiltrada = filtrarPorCheckbox(lista);
    listaFiltrada = buscarCoincidencia(filtro, listaFiltrada);
